  ...
  useEffect(() => {
    const loadInfluencer = async () => {
      ...
      try {
        ...
        const metricTrendRows = await fetchMetricTrend(influencerId).catch(() => []);
        const analysis = await fetchAverageCommentAnalysis(influencerId).catch((analysisFetchError) => {
          setAnalysisError(analysisFetchError instanceof Error ? analysisFetchError.message : String(analysisFetchError));
          return null;
        });
        setAverageAnalysis(analysis);

        const posts = await fetchPostActivityRows(influencerId).catch(() => []);
        ...
        if (currentInfluencer) {
          ...
          const currentScore = Math.round(
            normalizeLogScore(currentMetrics?.followers, SCORE_BENCHMARKS.followers) * 0.25 +
              normalizeLogScore(currentMetrics?.maximum_likes, SCORE_BENCHMARKS.maximumLikes) * 0.2 +
              normalizeLinearScore(currentMetrics?.posts, SCORE_BENCHMARKS.posts) * 0.15 +
              freshnessScore(currentPostSummary.latest_posted_at) * 0.2 +
              normalizeLinearScore(currentPostSummary.posting_span_days, SCORE_BENCHMARKS.postingSpanDays) * 0.1 +
        ...
        setRefreshJob(await fetchLatestRefreshJob(influencerId));
      } catch (loadError) {
