...
POST_LIMIT = env_int("INSTAGRAM_POST_LIMIT", 100000)

SLEEP_BETWEEN_CALLS_SEC = 0.2

# Apify
INSTAGRAM_COMMENT_ACTOR = os.getenv("INSTAGRAM_COMMENT_ACTOR", "apify~instagram-comment-scraper").strip()
INSTAGRAM_COOKIES = os.getenv("INSTAGRAM_COOKIES", "").strip()
INSTAGRAM_INCLUDE_REPLIES = os.getenv("INSTAGRAM_INCLUDE_REPLIES", "1").strip() == "1"
INSTAGRAM_MAX_COMMENTS = env_int("INSTAGRAM_MAX_COMMENTS", REL_MAX + TIME_MAX)
...
def apify_run_instagram_comments(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    url = f"https://api.apify.com/v2/acts/{INSTAGRAM_COMMENT_ACTOR}/run-sync-get-dataset-items"
...
def _extract_comment_items(raw_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for it in raw_items:
        ...
        if isinstance(it.get("comments"), list):
            ...
            continue
        if it.get("_type") in {"comment", "reply"}:
            out.append(it)
            continue
        if "text" in it and ("postUrl" in it or "postShortcode" in it or "commentId" in it or "id" in it):
            out.append(it)
    return out
...
def _normalize_comment(it: Dict[str, Any]) -> Dict[str, Any]:
    ...
    comment_id = it.get("id") or it.get("commentId")
    text = (it.get("text") or "").strip()
    like_count = (
        it.get("likeCount")
        or it.get("commentLikeCount")
        or it.get("likesCount")
        or 0
    )
    published_at = it.get("timestamp") or it.get("createdAt") or it.get("created_at")

    return {
        "comment_id": str(comment_id) if comment_id is not None else None,
        "commenter_id": str(owner.get("id")) if owner.get("id") is not None else None,
        "commenter_username": it.get("ownerUsername") or owner.get("username"),
        "commenter_display_name": owner.get("full_name"),
        "commenter_profile_pic_url": it.get("ownerProfilePicUrl") or owner.get("profile_pic_url"),
        "commenter_is_private": owner.get("is_private"),
        "commenter_is_verified": owner.get("is_verified"),
        "text": text,
        "like_count": int(like_count or 0),
        "published_at": published_at,
        "replies_count": int(it.get("repliesCount") or 0),
    }
...
def fetch_comments_sample(post_url: str) -> List[Dict]:
    payload: Dict[str, Any] = {
        "directUrls": [{"url": post_url}],
        "resultsLimit": int(INSTAGRAM_MAX_COMMENTS),
    }
    if INSTAGRAM_COOKIES:
        payload["cookies"] = INSTAGRAM_COOKIES

    try:
        raw = apify_run_instagram_comments(payload)
    except RuntimeError as e:
        msg = str(e)
        if "directUrls" in msg:
            payload["directUrls"] = [post_url]
            raw = apify_run_instagram_comments(payload)
        else:
            raise
    ...
    # Sample relevance by likes, time by timestamp
...
def is_spam_like(text: str) -> bool:
    ...
    # Very short generic-only (JP + EN) -> treat as spam-like
...
def get_instagram_posts(limit: int = 20) -> List[dict]:
    ig_accounts = (
        supabase.table("sns_accounts").select("id").eq("platform", PLATFORM).execute().data
    )
    account_ids = [a["id"] for a in ig_accounts]
    if not account_ids:
        return []
...
def process_post(post: dict) -> None:
    ...
    if not post_url and external_id:
        post_url = f"https://www.instagram.com/p/{external_id}/"
...
def main():
    posts = get_instagram_posts(limit=POST_LIMIT)
    if not posts:
    ...
    if os.getenv("RUN_ACCOUNT_AGGREGATE", "0").strip() == "1":
        try:
            subprocess.run(
                [sys.executable, "apify-scrapers/instagram/instagram_account_aggregate.py"],
                check=True,
            )
