...
def consistency_score_from_variance(var: Optional[float]) -> Optional[float]:
    """
    Turn variance into a 0..1 score (lower variance => higher consistency).
    This is a simple monotonic transform; tweak later.
    """
    if var is None:
        return None
    return 1.0 / (1.0 + var)
...
def get_youtube_account_ids(limit: int = 50) -> List[int]:
...
def merge_distributions(rows: List[dict], field: str, top_k: int = 10) -> Dict[str, float]:
...
def upsert_influencer_average(account_id: int, window_label: str, analyses: List[dict]) -> None:
    ...
    # Requires UNIQUE(account_id, window)
    supabase.table("influencer_average_comment_analysis").upsert(
        payload,
        on_conflict="account_id,window"
    ).execute()


def main():
    account_ids = get_youtube_account_ids(limit=50)
    if not account_ids:
        print("No YouTube accounts.")
        return
